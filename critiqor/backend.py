...
from .schemas import DiagnosisResult, EvidenceSubmission, validate_diagnosis_payload
...
def submit_evidence(submission: EvidenceSubmission, config: BackendConfig | None = None) -> DiagnosisResult:
    ...
    if run_id != submission.run_id:
        raise BackendResponseError(
            f"backend diagnosis run_id {run_id!r} does not match submitted run_id {submission.run_id!r}"
        )
    diagnosis.setdefault("run_id", run_id)
    errors = validate_diagnosis_payload(diagnosis)
    if errors:
        raise BackendResponseError(
            "backend returned an invalid diagnosis: " + "; ".join(errors[:8])
        )
